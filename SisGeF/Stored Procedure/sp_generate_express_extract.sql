CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_generate_express_extract`(
	in pdataInicio DATE,
    in pdataFinal DATE,
    in pvolumeExtra INT
)
...
where tbentregas.dat_baixa between pdataInicio and pdataFinal and tbentregas.dom_fechado <> 'S'
group by
